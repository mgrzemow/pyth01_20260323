...
class Spychacz:
    """Model prostego spychacza poruszającego się po siatce 2D."""
    ...
    def __init__(self, nr_rej: str) -> None:
        """Tworzy spychacz ustawiony w punkcie (0, 0), skierowany na północ."""
        self.nr_rej = nr_rej
        self._kierunek = 0
        self._x = 0
        self._y = 0

    def skret_w_prawo(self) -> None:
        """Obraca spychacz o 90 stopni w prawo."""
        self._kierunek = (self._kierunek + 1) % 4

    def wypisz(self) -> None:
        """Wypisuje bieżącą pozycję spychacza."""
        print(f'{self.__class__.__name__}("{self.nr_rej}") w ({self._x}, {self._y})')

    def __str__(self) -> str:
        """Zwraca opis pozycji i kierunku spychacza."""
    ...
    def __int__(self) -> int:
        """Zwraca przykładową reprezentację liczbową obiektu."""
        return 66
    ...
    def __eq__(self, other: object) -> bool:
        """Porównuje spychacze po numerze rejestracyjnym."""
        if not isinstance(other, Spychacz):
            return NotImplemented
        return self.nr_rej == other.nr_rej



    def jedz(self, n: int) -> None:
        """Przesuwa spychacz o ``n`` kroków w aktualnym kierunku."""
